...
> **Note:** These namespaces are auto-imported **only inside `CodedWorkflow`-derived classes**. In plain `.cs` files (helper classes, test files, or any class that does not extend `CodedWorkflow`), **all four namespaces above must be added explicitly** — this applies equally to portable (`IWordDocumentHandle`) and Windows/Interop (`IWordDocument`) usage:
> ```csharp
> using UiPath.Word;                        // core types: Position, Occurrence, etc.
> using UiPath.Word.Activities;             // activity-level types: ConflictBehavior, etc.
> using UiPath.Word.Activities.API;         // IWordDocumentHandle/IWordDocument extension methods, WordOperations static methods
> using UiPath.Word.Activities.API.Models;  // DocumentOptions, WordUseOptions, etc. (ConflictBehavior is in UiPath.Word.Activities)
> ```
> Without `using UiPath.Word.Activities.API;`, calls such as `doc.ReadText()`, `doc.AppendText(...)`, and `doc.ReplaceText(...)` fail with `CS1061`.
>
> **Name clash across Office packages — `ConflictBehavior`.** The Excel, Word, and PowerPoint coded APIs each declare their **own** `ConflictBehavior` enum — `UiPath.Excel.ConflictBehavior`, `UiPath.Word.Activities.ConflictBehavior`, and `UiPath.Presentations.Activities.ConflictBehavior` (the Excel and PowerPoint packages also each declare a `UseOptions` class; Word avoids that clash by naming its type `WordUseOptions`). If a single coded workflow references more than one of these packages, an unqualified `ConflictBehavior` becomes **ambiguous** and the compiler reports `CS0104: 'ConflictBehavior' is an ambiguous reference`. Resolve it by preferring the type from the **same package** as the options object you are configuring, written fully-qualified. For example, to replace an already-existing file via `DocumentOptions.ConflictBehavior`, use `UiPath.Word.Activities.ConflictBehavior.Replace` (or add an alias such as `using WordConflict = UiPath.Word.Activities.ConflictBehavior;`). The samples below use the fully-qualified form for this reason.
...
> **Alternative static-call form:** All extension methods above are declared on the `WordOperations` static class and can also be called in their explicit static form — e.g., `WordOperations.ReadText(doc)`. Prefer the extension method syntax (`doc.ReadText()`) in all normal usage; the static form is a fallback for contexts where extension method resolution fails (e.g., certain reflection-based tooling or F# interop).
...
Outside `CodedWorkflow`-derived classes none of the four namespaces are auto-imported. Add all four `using` directives explicitly so the `IWordDocumentHandle` extension methods and model types resolve.

```csharp
// Required in any plain .cs file (helper class, test, etc.) — none of these are auto-imported outside CodedWorkflow
using UiPath.Word;                        // core types: Position, Occurrence, etc.
using UiPath.Word.Activities;             // activity-level types: ConflictBehavior, etc.
using UiPath.Word.Activities.API;         // IWordDocumentHandle extension methods, WordOperations static methods
using UiPath.Word.Activities.API.Models;  // DocumentOptions, WordUseOptions, etc. (ConflictBehavior is in UiPath.Word.Activities)
...
public class DocumentHelper
{
    ...
    public DocumentHelper(IWordService word) => _word = word;

    public string ExtractText(string path)
    {
        using var doc = _word.UseDocument(path, createNew: false);
        // Extension methods on IWordDocumentHandle resolve because of the usings above
        return doc.ReadText();
    }
}
```

### Use the static-call form as a fallback (portable)

If extension method resolution is unavailable in your context (e.g., certain reflection-based tooling, generated code, or F# interop), all `IWordDocumentHandle` and `IWordDocument` extension methods can be called directly on the `WordOperations` static class. Prefer the extension method syntax in all normal usage.
...
public class DocumentProcessor
{
    ...
    public DocumentProcessor(IWordService word) => _word = word;

    public void Process(string templatePath, string outputPath)
    {
        // Work on a copy of the template so the original stays untouched
        File.Copy(templatePath, outputPath, overwrite: true);
        using var doc = _word.UseDocument(outputPath, createNew: false);

        // Static-call form — equivalent to doc.ReplaceText(...), doc.AppendText(...), doc.InsertDataTable(...)
        WordOperations.ReplaceText(doc, "{{Author}}", "Jane Smith");
        WordOperations.AppendText(doc, "Generated on: " + DateTime.Now.ToString("yyyy-MM-dd"));

        var summary = new DataTable();
        summary.Columns.Add("Item", typeof(string));
        summary.Columns.Add("Status", typeof(string));
        summary.Rows.Add("Section 1", "Complete");
        summary.Rows.Add("Section 2", "Pending");
        WordOperations.InsertDataTable(doc, summary, Position.End);
    }
}
```
...
public void Execute()
{
    using var doc = word.UseDocument(new DocumentOptions
    {
        ...
        CreateNew = true,
        ConflictBehavior = UiPath.Word.Activities.ConflictBehavior.Replace
    });
